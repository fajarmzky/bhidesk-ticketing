...
@section('content')
    <div class="box">
        ...
        <div class="box-header">
            ...
            <a onclick="addForm()" class="btn btn-primary" >Add Project</a>
            @endif
        </div>
                ...
                <tbody>
                    @foreach ($project as $item)
                        <tr>
                            ...
                            <td>PRJ0{{$item->id}}</td>
                            <td>{{$item->name}}</td>
                            <td>{{tanggal_local($item->start_date)}}</td>
                            <td>{{tanggal_local($item->end_date)}}</td>
                            <td>{{$item->nama}}</td>
                            <td>
                            @if(Auth::user()->role == "masteradmin")
...
@section('bot')
    ...
    <script type="text/javascript">
        ...
        $("#start_date").datepicker({ format: 'yyyy/mm/dd', autoclose: true, }).on('changeDate', function (selected) {
            ...
            var someFormattedDate = y + '/'+ mm + '/'+ dd;
                ...
                $('#end_date').datepicker('setEndDate', someFormattedDate);
            });

        $("#end_date").datepicker({ format: 'yyyy/mm/dd',autoclose: true,}).on('changeDate', function (selected) {
            ...
            var y = someDate.getFullYear();
            var someFormattedDate = y + '/'+ mm + '/'+ dd;
                $('#start_date').datepicker('setStartDate', someFormattedDate);
        ...
        $(function(){
            $('#modal-form form').validator().on('submit', function (e) {
                if (!e.isDefaultPrevented()){
                    var id = $('#id').val();
                    if (save_method == 'add') url = "{{ url('/addProject') }}";
                    else url = "{{ url('/updateProject')}}" + '/'  + id;
